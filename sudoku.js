...
hintBtn.dataset.action = 'hint';

document.getElementById('controls').appendChild(hintBtn);
...
Sudoku.prototype.hint = function() {
  ...
  inputs.forEach(input => {
    if(!input.value) emptyCells.push(input);
  });
  ...
  if(emptyCells.length > 0) {
    ...
    const solution = this.game.matrix.row[row][col];
    
    randomCell.value = solution;
    randomCell.classList.add('disabled');
    randomCell.tabIndex = -1;
